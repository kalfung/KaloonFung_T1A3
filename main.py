...
class Character:
    ...
    def take_damage(self, enemy):
        ...
        print(Fore.RED + f'The {enemy.name} attacks you for {damage} damage.')
    ...
    def heal(self):
        ...
        print(Fore.GREEN
              + f'You cast a healing spell and recover {heal_amount} health!')
        print(f'You now have {player.health} health.' + Style.RESET_ALL)
...
def spawn_coffer():
    ...
    with open('coffer.txt') as f:
        contents = f.read()
        print(contents)
    print('Do you want to ' + Fore.YELLOW + 'loot' + Style.RESET_ALL
          + ' the coffer, or ' + Fore.RED + 'ignore' + Style.RESET_ALL
          + ' it and continue?')
    while True:
        ...
        if decision == 'ignore':
            ...
            print(Fore.YELLOW
                  + f'You find {coffer.gold} pieces of gold inside.')
            coffer.loot(player)
...
def spawn_mimic():
    ...
    while True:
        ...
        if decision == 'ignore':
            print(Fore.GREEN
                  + 'Good choice. That was actually a mimic in disguise!')
            print('You get away safely!' + Style.RESET_ALL)
            break
        elif decision == 'loot':
            print(f'The {enemy.name} reveals itself!')
            print('It sheds its glittering disguise and attacks!')
            ...
            while player.health > 0 and enemy.health > 0:
                if players_turn:
                    print('Do you ' + Fore.RED + 'attack' + Style.RESET_ALL
                          + f' the {enemy.name} or cast a ' + Fore.GREEN
                          + 'heal' + Style.RESET_ALL + ' spell?')
                    decision = input().lower()
                    if decision == 'heal':
                        player.heal()
            ...
            if player.health <= 0:
                print(Fore.RED
                      + f'You have been defeated by the {enemy.name}.')
                game_recap()
            else:
                print(Fore.YELLOW
                      + f'You have successfully defeated the {enemy.name}!'
                      + Style.RESET_ALL)
                enemies_defeated += 1
            break
        else:
...
def spawn_monster():
    ...
    if player.health <= 0:
        ...
        print(Fore.YELLOW + f'You have successfully defeated the {enemy.name}!'
              + Style.RESET_ALL)
        enemies_defeated += 1
...
def game_recap():
    print(Fore.CYAN + f'You encountered {enemies_encountered} enemies.')
    print(f'You defeated {enemies_defeated} of those enemies.')
    print(f'You encountered {coffers_encountered} coffers.')
    print(f'You looted {coffers_looted} coffers.' + Style.RESET_ALL)
    sys.exit()
...
print('Armed with your trusty sword and handful of spells,')
print('you venture bravely into the dark depths before you.')
art.aprint('sword6')
while True:
    print('Continue to the next area?')
    decision = input(Fore.GREEN + 'Y' + Style.RESET_ALL + ' or '
                     + Fore.RED + 'N? ' + Style.RESET_ALL).lower()
    if decision == 'n':
        ...
        if encounter >= 9:
            ...
            if coffer_drop >= 9:
                print(Fore.YELLOW + 'Huzzah! '
                      + 'The defeated monster drops a bonus treasure coffer!'
                      + Style.RESET_ALL)
                coffers_encountered += 1
                spawn_coffer()
            else:
            ...
            print(Fore.YELLOW
                  + 'You discover a suspicious looking treasure coffer.'
                  + Style.RESET_ALL)
            enemies_encountered += 1
            spawn_mimic()
    else:
